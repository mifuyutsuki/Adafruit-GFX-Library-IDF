This is a version of Adafruit GFX for ESP-IDF purists, stripping the library of Arduino dependencies (including Arduino-dependent Adafruit_BusIO). Some functionality is disabled, such as:

- GrayOLED and SPITFT functions (depends on Arduino `Wire` or `SPI`)
- `getTextBounds()` (depends on Arduino `String`)
- `drawButton()` (depends on Arduino `Print`)
